...
active_horizon:
  ...
  - LAB-BRIDGE-01
  - LAB-DEMO-01
product_state: product PR #187 head 53f0cfda is exact-head hosted-green with the canonical fixture;
  product merge must precede lab PR #8
capabilities:
...
blockers: merge product PR #187 first; then perform the exact merged-schema byte check and merge
  lab PR #8 after its final docs-only head gate and aging floor
...
exact_resume_point: merge product PR #187 after its aging floor; verify the merged product commit
  retains schema SHA-256 634b0cc7 and fixture SHA-256 26c3a918; then merge lab PR #8 and close the
  bounded programme before considering product #174
```
