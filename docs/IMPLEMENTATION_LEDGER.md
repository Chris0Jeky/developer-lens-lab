...
## 2026-08-15 - Release-authority truth repair (issue #29)

- Corrected the three operative Lab surfaces that assigned joint-tag mechanics to the owner:
  `docs/agent-system/CROSS_REPO_CONTRACT.md`,
  `docs/agent-system/MAINTENANCE_PROTOCOL.md`, and `docs/CURRENT_STATE.md`. They now implement the
  constitution's A1=FULL split: agents produce the visual proof; the owner supplies only the final
  five-minute aesthetic decision at
  `Chris0Jeky/developer-lens::HUMAN_TODO.md::q-10(c)`; after Product records that sign-off, agents
  execute synchronized version, tag, package, and approved C0-publication mechanics under the
  ordinary release gates.
- Preserved the exact scope of closed
  `Chris0Jeky/developer-lens-lab::HUMAN_TODO.md::q-11`: approval of the delivered Lab media package
  only. Product browser visual QA remains NOT VERIFIED / PARKED until the mandated browser surface
  becomes available, as recorded by Product issue #200 comment `5299321093`. This repair claims no
  Product QA proof, Product sign-off, tag, package publication, or C0-asset publication.
- Older ledger passages that said the joint tag returned to the owner or stayed owner-executed are
  retained as immutable history but superseded for current authority by owner constitution A1=FULL
  and this repair; they must not be used as execution instructions.
- Recorded the surprising authority divergence as FR-084, linked to issue #29 comment
  `5299376876`. A read-only inline `gh --jq` filter also reproduced FR-009's quoting seam; the
  compatible PowerShell JSON route succeeded without mutation and occurrence 7 is recorded in the
  same hop. FR-084 is the next free identifier on live main; parked PR #87 must renumber its
  independently reserved FR-084 if resumed.
- Proof for this documentation/authority milestone: the context verifier and `git diff --check`
  passed first, followed by the complete locked gate declared in `CLAUDE.md`: doctor/context,
  formatting, lint, pyright (0 errors), pytest (293 passed), strict MkDocs, hygiene, and final diff
  check were green. No protected or generated bytes were inspected, `uv.lock` was not modified,
  and no data, model, telemetry, credential, release, publication, or owner-gate authority changed.

## 2026-08-15 - Release-asset immutable provenance pins (issue #76, PR #86; recorded belatedly)

- PR #86 merged 2026-08-14T23:30:28Z as `2806574915e80118e43dee577bf0c53ea0d1fc83`, changing only
  `release-assets/v0.1.0/method-trial-v1/provenance.json` and `tests/test_release_assets.py`. The
  staged manifest now carries immutable implementation provenance: the frozen producer commit
  `0ef193070a9b80b81cef5a1710a1d65e0b271c15`, the verified Lab commit
  `89358200b428aac53d1c8b47a3d544e7a981efac`, and four path→blob OID pins for the renderer,
  validator, serializer, and schema. The test resolves those Git objects directly instead of
  executing mutable implementation, and fails closed on missing commits or paths, non-blob
  objects, or OID mismatch.
- Proof recorded on the pull request: hosted `Prove the lab` green at the head and at the merge
  commit (run `31850603185`), an exact-head review, and a clean delayed post-merge sweep at PR #86
  comment `5299230783`. Independently, the 2026-08-15 full local gate at main
  `0fd8a50a39bb6632e21982c33c6a5c409a6fcf6f` (310 passed) exercises the shipped test.
- This ledger and `docs/CURRENT_STATE.md` previously omitted the milestone; the 2026-08-15
  post-wave reconciliation records it belatedly. Tracked staging only — no publication, release,
  or tag is authorized.

## 2026-08-15 - Merge-eligibility predecessor review history (PR #89; recorded belatedly)

- PR #89 merged 2026-08-15T01:23:38Z as `41b4f23358b570d6c20740cb7f27dcffe246c688`, changing
  `tools/merge_eligibility.py`, `tests/test_merge_eligibility.py`, and the "Predecessor review
  history stays complete" section of `docs/agent-system/CONTINUOUS_WORK_PROTOCOL.md`. Formal
  reviews and top-level comments bound to a predecessor head are retained as complete append-only
  history and tolerated only in normalized state `APPROVED`/`COMMENTED`/`DISMISSED`;
  `CHANGES_REQUESTED`, `PENDING`, absent or unknown states, wrong pull-request or base bindings,
  and unresolved review threads remain fail-closed, and the `accepted_review` attestation still
  requires the exact current head, current base, and pull-request identity.
- Proof recorded on the pull request: hosted `Prove the lab` green at the head and at the merge
  commit (run `31856385187`) and an exact-head review at PR #89 comment `5299742281`. The delayed
  post-merge sweep is backfilled clean at issue #29 comment `5302113363`. Independently, the
  2026-08-15 full local gate at main `0fd8a50a39bb6632e21982c33c6a5c409a6fcf6f` (310 passed)
